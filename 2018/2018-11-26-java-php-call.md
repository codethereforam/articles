...
## 三、如何提供服务给PHP系统
### 3.1 Dubbo(阿里开源)
#### 3.1.1 Dubbo + [dubbo-php-framework](https://github.com/dubbo/dubbo-php-framework/wiki)
##### 特点
可以实现php调用php，php调用java(dubbo)，java(dubbo)调用php，Java调Java，真正的跨语言
##### 缺点
开发活跃程度极低，社区小，资料少，没有release版本
#### 3.1.2 Dubbo Restful（合并自Dubbox）
##### 特点
显著简化企业内部的异构系统之间的（跨语言）调用。一定程度简化dubbo系统对其它异构系统的调用。
...
### 3.2 montan（微博开源）
#### 3.2.1 motan + [motan-php](https://github.com/weibocom/motan-php)
...
#### 3.2.2 [motan Restful](https://github.com/weibocom/motan/wiki/zh_quickstart#%E4%BD%BF%E7%94%A8restful%E5%8D%8F%E8%AE%AE)
模仿Dubbo Restful，很类似，从文档上对比，不如Dubbo Restful
#### 3.2.3 Client + Agent(还未完全开源)
大概是使用motan + motan-go(agent)等实现。确定weibo mesh的服务化方向，将 Motan Agent定位为统一服务管理的执行节点。
#### 实现方式
